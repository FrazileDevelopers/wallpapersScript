...
if(len(pages_to_download.split('-')) < 2):
    firstpage = 1
    lastpage = int(f.getFileName(soup.findAll(
        "a", {"class": "pager__link"})[2]['href']).replace("page", ""))
else:
...
for i in range(firstpage, lastpage+1):
    download_thread = threading.Thread(target=f.downloadAllFromPageLink, args=(
        mainLink+"/page"+str(i), resolution, foldername, lastpage*15,))
